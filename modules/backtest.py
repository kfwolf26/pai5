...
# 排列5 - 5 个位置键 + 中文短名(与 prediction_model / statistics 保持一致)
BT_POS_KEYS = ["w", "q", "b", "s", "g"]
BT_POS_NAMES = ["万", "千", "百", "十", "个"]
...
class RandomStrategy(BaseStrategy):
    ...
    def predict(self, train_data, top_n):
        n = min(top_n, 10)
        return {pk: sorted(random.sample(range(10), n)) for pk in BT_POS_KEYS}
...
class FrequencyStrategy(BaseStrategy):
    ...
    def predict(self, train_data, top_n):
        result = {}
        for pos, key in enumerate(BT_POS_KEYS):
...
class BacktestEngine:
    ...
    def _filter_valid(self):
        """过滤掉 number 字段不合法(空/非数字)的记录。排列5: 5 位"""
        valid = []
    ...
    def run(self, strategy: BaseStrategy, top_n: int = 5,
        ...
        while start + train_size + test_size <= n:
            ...
            for record in test:
                ...
                eval_result["predicted"] = {pk: pred[pk] for pk in BT_POS_KEYS}
    ...
    def predict_next(self, strategy: BaseStrategy, top_n: int = 5,
        ...
        return {
            "issue": next_issue,
            "predicted": {pk: pred[pk] for pk in BT_POS_KEYS},
            "actual": "",
            "hit_count": -1,
            "hit_pos": [],
            "is_prediction": True,
...
class BacktestTab:
    ...
    def _build_comparison_tab(self):
        ...
        columns = (["策略", "总期数", "平均命中位", "0位占比", "1位占比",
                   "2位占比", "3位占比", "4位占比", "5位全中"]
                  + [f"{n}位命中" for n in BT_POS_NAMES])
        self.compare_tree = ttk.Treeview(tab, columns=columns, show="headings", height=12)
        widths = {"策略": 140, "总期数": 70, "平均命中位": 90, "0位占比": 70,
                  "1位占比": 70, "2位占比": 70, "3位占比": 70, "4位占比": 70, "5位全中": 80}
        for n in BT_POS_NAMES:
            widths[f"{n}位命中"] = 75
        for c in columns:
            self.compare_tree.heading(c, text=c)
            self.compare_tree.column(c, width=widths.get(c, 75), anchor=tk.CENTER)
    ...
    def _build_detail_tab(self):
        ...
        for tag_name, fg, bold in tag_specs:
            font_spec = ("微软雅黑", 9, "bold") if bold else ("微软雅黑", 9)
            self.detail_tree.tag_configure(tag_name, foreground=fg, font=font_spec)
    ...
    def _render_summary(self, result: Dict):
        ...
        for k in range(len(BT_POS_KEYS) + 1):
            cnt = m['hit_distribution'].get(k, 0)
            rate = m['hit_distribution_rate'].get(k, 0) * 100
            label = "全错" if k == 0 else (f"{len(BT_POS_KEYS)}位全中" if k == len(BT_POS_KEYS) else f"{k}位命中")
            lines.append(f"  {label}:  {cnt:>5} 期 ({rate:>5.1f}%)")

        lines += [
            "",
            f"📍 各位置命中率 (随机期望: {top_n*10:.0f}%)",
        ]
        for n in BT_POS_NAMES:
            rate = m['pos_rates'].get(n, 0) * 100
            lines.append(f"  {n}位命中:  {rate:.1f}%")
        ...
        lines += [
            ...
            f"(随机期望: {(top_n/10)**len(BT_POS_KEYS)*100:.4f}%)",
            "",
            "=" * 60,
            "💡 解读:",
            f"  - Top-{top_n} 即每位保留 {top_n} 个号,理论随机命中 1 位的概率 ~{top_n*10:.0f}%",
            f"  - 5位全中(直选)理论概率 {(top_n/10)**len(BT_POS_KEYS)*100:.4f}%,需长期跟踪才有意义",
    ...
    def _render_comparison(self, comparison: Dict, top_n: int):
        ...
        for name, result in comparison.items():
            if "error" in result:
                # 错误行:策略名 + 全 0
                err_row = [name, 0, 0, "错误", "", "", "", "", ""] + [""] * len(BT_POS_NAMES)
                rows.append(tuple(err_row))
                continue
            m = result.get("metrics", {})
            if not m:
                continue
            row = [
                ...
                f"{m['hit_distribution_rate'].get(3, 0)*100:.1f}%",
                f"{m['hit_distribution_rate'].get(4, 0)*100:.1f}%",
                f"{m['zhixuan_count']} ({m['zhixuan_rate']*100:.2f}%)",
            ]
            for n in BT_POS_NAMES:
                row.append(f"{m['pos_rates'].get(n, 0)*100:.1f}%")
    ...
    def _insert_detail_row(self, r: Dict):
        """向 detail_tree 插入单行(预测行或历史行) - 5 位版"""
        is_pred = r.get("is_prediction", False)
        pred = r.get("predicted", {})
        actual = r.get("actual", "")

        # 5 位预测字符串
        pos_strs = [" ".join(str(x) for x in pred.get(pk, [])) for pk in BT_POS_KEYS]

        if is_pred:
            values = [f"🔮 {r.get('issue', '')} (预测)", "—"] + pos_strs + ["待开奖", "下一期预测 · 基于全部历史训练"]
            self.detail_tree.insert("", tk.END, values=values, tags=("prediction",))
        else:
            hit_count = r.get("hit_count", 0)
            # tag 限制在 0..5
            tag_idx = min(max(hit_count, 0), len(BT_POS_KEYS))
            tags = (f"hit{tag_idx}",)
            detail_parts = []
            for i, pk in enumerate(BT_POS_KEYS):
                pos_name = BT_POS_NAMES[i]
                if i < len(actual):
                    hit = int(actual[i]) in pred.get(pk, [])
                    detail_parts.append(f"{pos_name}={'✓' if hit else '✗'}")
                else:
                    detail_parts.append(f"{pos_name}=?")
            values = [r.get("issue", ""), actual] + pos_strs + [f"{hit_count}位", " ".join(detail_parts)]
    ...
    def _render_analysis(self, strategy_name: str, top_n: int, train_size: int):
        ...
        pos_names = [f"{n}位" for n in BT_POS_NAMES]
        ...
        lines.append(f"   比如{BT_POS_NAMES[0]}位热号 {hot[:3]},策略更倾向选它们")
