...
# 排列5 - 5 个位置键 + 展示名
PM_POS_KEYS = ["w", "q", "b", "s", "g"]
PM_POS_LABELS = {"w": "万位", "q": "千位", "b": "百位", "s": "十位", "g": "个位"}
PM_POS_SHORT = {"w": "万", "q": "千", "b": "百", "s": "十", "g": "个"}
...
class PredictionModel:
    ...
    def _save_remain_numbers(self):
        ...
        remain = {pk: [] for pk in PM_POS_KEYS}

        if next_issue and next_issue in predictions:
            pos_idxs = {pk: i for i, pk in enumerate(PM_POS_KEYS)}
            kill_sets = {pk: set() for pk in PM_POS_KEYS}
            ...
            for pos_key in PM_POS_KEYS:
                pos_idx = pos_idxs[pos_key]
                    ...
                    for seq in range(predict_count):
                        ...
                        for hist_issue in history_issues:
                            ...
                            if pid < len(predictors):
                                pred = predictors[pid]
                                if pos_key in pred and seq < len(pred[pos_key]):
                                    pred_num = pred[pos_key][seq]
                                    result_pos = int(result_num[pos_idx]) if pos_idx < len(result_num) else -1
                        ...
                        if streak >= min_streak:
                            ...
                            if pid < len(next_predictors):
                                ...
                                if pos_key in next_pred and seq < len(next_pred[pos_key]):
                                    kill_sets[pos_key].add(next_pred[pos_key][seq])

            for pk in PM_POS_KEYS:
                remain[pk] = sorted(set(range(10)) - kill_sets[pk])
        else:
            for pk in PM_POS_KEYS:
                remain[pk] = list(range(10))
        ...
        hit_pos = []
        if result:
            for i, pk in enumerate(PM_POS_KEYS):
                if i < len(result) and int(result[i]) in remain[pk]:
                    hit_pos.append(PM_POS_SHORT[pk])
    ...
    def _refresh_results(self):
        ...
        for rec in self.saved_predictions:
            ...
            # 兼容老结构(bai_remain/shi_remain/ge_remain)与新结构(remains={w,q,b,s,g})
            remains = rec.get("remains")
            if not remains:
                remains = {
                    "w": rec.get("bai_remain", []),
                    "q": rec.get("shi_remain", []),
                    "b": rec.get("ge_remain", []),
                    "s": [],
                    "g": []
                }
            ...
            if not new_result:
                ...
                hit_pos = []
                for i, pk in enumerate(PM_POS_KEYS):
                    if i < len(new_result) and int(new_result[i]) in remains.get(pk, []):
                        hit_pos.append(PM_POS_SHORT[pk])
                rec["hit_pos"] = hit_pos
                updated_count += 1
    ...
    def _calc_prediction_stats(self):
        ...
        # 5 位命中率
        pos_hits = {pk: 0 for pk in PM_POS_KEYS}
        hit_len_dist = {k: 0 for k in range(len(PM_POS_KEYS) + 1)}  # 0..5
        ...
        for rec in self.saved_predictions:
            ...
            for pk in PM_POS_KEYS:
                if PM_POS_SHORT[pk] in hit_pos:
                    pos_hits[pk] += 1
            hit_len_dist[min(len(hit_pos), len(PM_POS_KEYS))] += 1

        pos_rates = {pk: f"{pos_hits[pk] / total * 100:.1f}%" for pk in PM_POS_KEYS}
        all_hit = hit_len_dist.get(len(PM_POS_KEYS), 0)
        none_hit = hit_len_dist.get(0, 0)
        four_hit = hit_len_dist.get(4, 0)
        all_rate = f"{all_hit / total * 100:.1f}%"
        none_rate = f"{none_hit / total * 100:.1f}%"
        four_rate = f"{four_hit / total * 100:.1f}%"

        pos_strs = [f"{PM_POS_LABELS[pk]}命中 {pos_hits[pk]}/{total} ({pos_rates[pk]})" for pk in PM_POS_KEYS]
        stats_text = (f"已开奖 {total} 期 | " +
                      " | ".join(pos_strs) +
                      f" | 五位全中 {all_hit}/{total} ({all_rate}) | 四位命中 {four_hit}/{total} ({four_rate}) | 全未中 {none_hit}/{total} ({none_rate})")
        ...
        ttk.Label(row1, text=f"五位全中：{all_hit} 期 ({all_rate})", foreground="#d32f2f", font=("微软雅黑", 10, "bold")).pack(side=tk.LEFT, padx=10)
        ttk.Label(row1, text=f"四位命中：{four_hit} 期 ({four_rate})", foreground="#f57c00", font=("微软雅黑", 10, "bold")).pack(side=tk.LEFT, padx=10)
        ...
        for pk in PM_POS_KEYS:
            ttk.Label(row2, text=f"{PM_POS_LABELS[pk]}：{pos_hits[pk]}/{total} ({pos_rates[pk]})", font=("微软雅黑", 10)).pack(side=tk.LEFT, padx=10)

        list_frame = ttk.LabelFrame(top, text="详细记录", padding=5)
        list_frame.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))

        columns = ["issue", "result"] + [f"{pk}_remain" for pk in PM_POS_KEYS] + [PM_POS_SHORT[pk] for pk in PM_POS_KEYS]
        tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=15)
        tree.heading("issue", text="期号")
        tree.heading("result", text="开奖号")
        for pk in PM_POS_KEYS:
            tree.heading(f"{pk}_remain", text=f"{PM_POS_LABELS[pk]}剩余")
            tree.heading(PM_POS_SHORT[pk], text=PM_POS_SHORT[pk])
        tree.column("issue", width=80, anchor=tk.CENTER)
        tree.column("result", width=90, anchor=tk.CENTER)
        for pk in PM_POS_KEYS:
            tree.column(f"{pk}_remain", width=90, anchor=tk.CENTER)
            tree.column(PM_POS_SHORT[pk], width=40, anchor=tk.CENTER)
        ...
        for rec in reversed(self.saved_predictions):
            issue = rec.get("issue", "")
            # 兼容老结构: remains={w,q,b,s,g} 或 老的 bai_remain/shi_remain/ge_remain
            remains = rec.get("remains")
            if not remains:
                remains = {
                    "w": rec.get("bai_remain", []),
            ...
            tags = ()
            if result and result != "待开奖" and len(hit_pos) == len(PM_POS_KEYS):
                tags = ("all_hit",)

            hit_labels = []
            for pk in PM_POS_KEYS:
                short = PM_POS_SHORT[pk]
                hit_labels.append("中" if short in hit_pos else ("错" if result and result != "待开奖" else ""))

            values = [issue, result]
            for pk in PM_POS_KEYS:
                values.append(" ".join(str(n) for n in remains.get(pk, [])))
            values.extend(hit_labels)
    ...
    def _build_ui(self):
        ...
        query_columns = ("issue", "result") + tuple(f"{pk}_pred" for pk in PM_POS_KEYS)
        ...
        for pk in PM_POS_KEYS:
            self.query_tree.heading(f"{pk}_pred", text=f"{PM_POS_LABELS[pk]}预测")
        self.query_tree.column("issue", width=80, anchor=tk.CENTER)
        self.query_tree.column("result", width=90, anchor=tk.CENTER)
        for pk in PM_POS_KEYS:
            self.query_tree.column(f"{pk}_pred", width=110, anchor=tk.CENTER)
        ...
        positions = [(pk, f"位置{i + 1}（{PM_POS_LABELS[pk]}）") for i, pk in enumerate(PM_POS_KEYS)]

        for i in range(len(PM_POS_KEYS)):
        ...
        try:
            ...
            def do_gen():
                ...
                for i, record in enumerate(recent_history):
                    ...
                    for pid in range(predictor_count):
                        predictor_pred = {"predictor_id": pid + 1}
                        for pk in PM_POS_KEYS:
                            predictor_pred[pk] = random.sample(range(10), min(predict_count, 10))
    ...
    def _query_predictor_data(self):
        ...
        for issue in issues:
            ...
            if pid < len(predictors):
                pred = predictors[pid]
                # 兼容新旧结构: 新 key=w/q/b/s/g, 旧 key=bai/shi/ge
                if "bai" in pred:
                    pred_aliased = {"w": pred["bai"], "q": pred["shi"], "b": pred["ge"], "s": [], "g": []}
                else:
                    pred_aliased = pred

                pred_lists = [pred_aliased.get(pk, []) for pk in PM_POS_KEYS]
                pos_results = [result_num[i] if i < len(result_num) else "" for i in range(len(PM_POS_KEYS))]

                pred_strs = []
                for plist, pres in zip(pred_lists, pos_results):
                    pred_strs.append(",".join(
                        (str(num) + "★") if (result_num and str(num) == pres) else str(num)
                        for num in plist
                    ))
                ...
                if not result_num:
                    ...
                    all_hit = all(
                        any(str(num) == pres for num in plist) if pres else True
                        for plist, pres in zip(pred_lists, pos_results)
                    )
                    if all_hit:
                        tags = ("hit_row",)

                self.query_tree.insert("", tk.END, values=(issue, display_result) + tuple(pred_strs), tags=tags)
    ...
    def _calculate_kill_numbers(self):
        ...
        positions = [(pk, f"位置{i + 1}") for i, pk in enumerate(PM_POS_KEYS)]
        pos_idx_map = {pk: i for i, pk in enumerate(PM_POS_KEYS)}
        pos_count = len(PM_POS_KEYS)
        ...
        def do_calc():
                ...
                for pid in range(predictor_count):
                    ...
                    for issue in history_issues:
                        ...
                        if pid < len(predictors):
                            ...
                            # 兼容旧数据
                            if "bai" in pred:
                                pred_map = {"w": pred["bai"], "q": pred["shi"], "b": pred["ge"], "s": [], "g": []}
                            else:
                                pred_map = pred
                            pidx = pos_idx_map[pos_key]
                            result_pos_num = int(result_num[pidx]) if pidx < len(result_num) else -1
                            if result_pos_num in pred_map.get(pos_key, []):
                                total_hits += 1
            ...
            for pos_idx, (pos_key, pos_name) in enumerate(positions):
                    ...
                    for seq in range(predict_count):
                        ...
                        for issue in history_issues:
                            ...
                            if pid < len(predictors):
                                ...
                                if "bai" in pred:
                                    ...
                                    pred_map = pred
                                pk_list = pred_map.get(pos_key, [])
                                if seq < len(pk_list):
                                    pred_num = pk_list[seq]
                                    result_pos_num = int(result_num[pidx]) if pidx < len(result_num) else -1
                        ...
                        if streak >= min_streak:
                            if pid < len(next_predictors):
                                next_pred = next_predictors[pid]
                                if "bai" in next_pred:
                                    next_map = {"w": next_pred["bai"], "q": next_pred["shi"], "b": next_pred["ge"], "s": [], "g": []}
                                else:
                                    next_map = next_pred
                                pk_list = next_map.get(pos_key, [])
                                if seq < len(pk_list):
                                    kill_num = pk_list[seq]
                ...
                self.root.after(0, lambda p=pos_idx + 1, t=pos_count: self.status_var.set(f"正在计算杀号... {p}/{t}"))
            ...
            def update_ui():
                for pos_key, pos_name in positions:
                    ...
                    if pos_key not in self.kill_trees:
                        continue
        ...
        self.status_var.set(f"正在计算杀号... 0/{pos_count}")
